      ...
      <div className="flex flex-col gap-3 w-full h-full justify-center">
        {config.showAQI !== false && (
          <div className="flex items-center gap-3">
            <span className={`${aqiColor} text-white text-sm font-bold px-3 py-1 rounded-full`}>
              AQI {data.aqi}
            </span>
            <span className="opacity-80 text-sm">{aqiLabel}</span>
          </div>
        )}

        {config.showPollutants && (
          <div className="flex flex-wrap gap-x-4 gap-y-1 opacity-70 text-sm">
